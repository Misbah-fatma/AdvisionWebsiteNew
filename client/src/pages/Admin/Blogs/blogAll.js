...
const BlogDetail = () => {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        const response = await axiosInstance.get(`/blogsall/${id}`);
      ...
      <section id="blog-detail" className="blog-detail">
        <div className="container mt-2">
          <div className="section-title">
            <div className="btn btn-sm border rounded-pill text-primary px-3 mb-3 mt-4">Blog Details</div>
            <h2 className="mb-4">{blog.heading}</h2>
            <p className="mb-4 text-center">{blog.description}</p>
          </div>
