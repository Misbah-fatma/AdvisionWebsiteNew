...
const BlogDetail = () => {
  const { channelid } = useParams(); // Get blog ID from route parameters
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        const response = await axiosInstance.get(`/getchannelsAll/${channelid}`);
      ...
      <section id="blog-detail" className="blog-detail">
        <div className="container mt-2">
          <div className="section-title">
            <div className="btn btn-sm border rounded-pill text-primary px-3 mb-3 mt-4">Channels Details</div>
            <h2 className="mb-4">{blog.title}</h2>
            <p className="mb-4 text-center">{blog.text}</p>
          </div>
